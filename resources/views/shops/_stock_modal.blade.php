...
@else
    <div class="text-center mb-3">
        ...
        @if ($stock->purchase_limit)
            <div class="text-danger">Max {{ $stock->purchase_limit }} @if ($stock->purchase_limit_timeframe !== 'lifetime') {{ $stock->purchase_limit_timeframe }} @endif per user</div>
        @endif
    ...
    @if (Auth::check())
        @if (($stock->is_fto && Auth::user()->settings->is_fto) || !$stock->is_fto)
            ...
            @elseif($purchaseLimitReached)
                <div class="alert alert-warning mb-0">You have already purchased the limit of {{ $stock->purchase_limit }} of this item @if ($stock->purchase_limit_timeframe !== 'lifetime') within the
                        {{ $stock->purchase_limit_timeframe }} reset @endif.</div>
            @else
                @if ($stock->purchase_limit)
                    <div class="alert alert-warning mb-3">You have purchased this item {{ $userPurchaseCount }} times @if ($stock->purchase_limit_timeframe !== 'lifetime') within the {{ $stock->purchase_limit_timeframe }} reset
                        @endif.</div>
                @endif
